...
const SCHOOL_IMPORT_CONFIG = {
  public: {
    ...
    hasWing: true,
    // Public Schools records come from the government EMIS register and
    // are never created through this portal (the "Add School" button is
    // hidden here for the same reason) — so this import can only UPDATE
    // fields on an Emis that already exists, never insert a new school.
    updateOnly: true,
    // Only the match key is strictly required for an update; any other
    // column left blank in the sheet simply isn't touched on that row.
...
function openSchoolImportModal(kind) {
  ...
  document.getElementById('si_title').innerHTML =
    `<i class="bi bi-file-earmark-arrow-up"></i> ${cfg.updateOnly ? 'Update' : 'Import Previous'} ${cfg.label} Data`;
  document.getElementById('si_instructions').textContent = cfg.instructions;
...
async function schoolImportGoToPreview() {
  ...
  _siPreviewRows = _siRawRows.map(raw => {
    ...
    else if (cfg.updateOnly && !exists) status = 'notfound';   // public: no Add path, so an unrecognized Emis is skipped
    else if (!cfg.updateOnly && exists) status = 'duplicate';  // private: already imported, don't duplicate
  ...
  document.getElementById('si_previewSummary').innerHTML = `
    <span style="color:var(--ok)"><i class="bi bi-check-circle"></i> ${counts.ok || 0} ready to ${cfg.updateOnly ? 'update' : 'import'}</span>
    ${cfg.updateOnly
      ? `&nbsp;·&nbsp; <span style="color:var(--warn)"><i class="bi bi-question-circle"></i> ${counts.notfound || 0} Emis not found (will be skipped — this only updates existing schools)</span>`
  ...
  const statusLabel = { ok: cfg.updateOnly ? '✓ Will update' : '✓ Ready', duplicate: 'Duplicate', notfound: 'Emis not found', outside: 'Outside jurisdiction' };
...
async function confirmSchoolImport() {
  ...
  for (const { row } of toApply) {
    ...
    if (cfg.updateOnly) {
      // Never touch the match key itself, and never insert — this can
      // only update fields on a school that's already in the system.
      const key = dbRow[cfg.uniqueCol];
      delete dbRow[cfg.uniqueCol];
      const { error } = await _sb.from(cfg.table).update(dbRow).eq(cfg.uniqueCol, key);
